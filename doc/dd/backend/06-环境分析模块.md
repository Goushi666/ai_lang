...
| GET | `/environment/export` | **统一导出**：Query 含 `start_time`、`end_time`、`device_id?`；**format**=`json`\|`csv`；**scope** 控制导出范围（见 OpenAPI）；返回 JSON 或 `text/csv` |
| GET | `/environment/anomalies/export` | 同上 Query；`text/csv`（异常片段专用，与摘要 anomalies 语义一致） |

### 7.1 环境监测路由（`/api/monitoring`）

由 `app/api/v1/monitoring.py` 提供，供前端「环境监测」页使用（部分能力与环境分析共享仓库）：

| 方法 | 路径 | 说明 |
|------|------|------|
| GET | `/temperature-timeline` | 温度时间线（实测+预测等） |
| GET | `/anomalies/records` | 分页查询已落库异常记录 |
| POST | `/anomalies/records/batch-delete` | 批量删除 |
| DELETE | `/anomalies/records/{record_id}` | 单条删除 |
| GET | `/anomalies` | 过去 24h 内异常片段（分析服务） |

**MonitoringService**（`app/services/monitoring_service.py`）：封装监测页所需时间线等读逻辑。  
**EnvironmentAnomalyRepository**（`app/repositories/environment_anomaly_repo.py`）：异常落库的列表/删除等。
